...
export default function FilmDetail() {
    ...
    const fetchCinemas = async () => {
      try {
        const response = await fetch(
          `/api/film/filmInfo/id=${film_id}/lichChieu/khuVuc_id=${selectedArea?.region_id}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleNavigate = (showtime_id) => {
    ...
    navigate(`/dat_ve/${film_name}`,{ state: { showtime_id: showtime_id } });
  };
    ...
    <>
      ...
      {data &&
        (() => {
          ...
          const exactlyDate =
            item.Release_date.substring(8, 10) +
            "/" +
            item.Release_date.substring(5, 7) +
            "/" +
