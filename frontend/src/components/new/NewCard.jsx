...
export default function NewCard(props) {
  ...
  return (
    <Box sx= {{width: '100%', height: '100%', cursor: 'pointer'}} onClick={() => ClickNew(props.new_id, props.header)}>
    ...
    WebkitLineClamp: 2,}}>{props.footer}</div>
        <div style={{textAlign: 'end',  fontSize: '15px', color: theme.palette.mode === 'dark' ? '#cdd1cf' : '#7e8280'}}>{props.date}</div>
    </Box>
  )
}
